...
SELECT Cli_Email, Cli_Prenom, CLi_Nom FROM Client NATURAL JOIN (
SELECT IDClient FROM Client NATURAL JOIN Commande WHERE Com_DateLivraison >= (
SELECT LF_DateArrivee FROM LivraisonFournisseur NATURAL JOIN ProduitParLivraison WHERE IDProduit IN (
SELECT IDProduit FROM Produit WHERE Pro_Nom = "congue" ) AND IDEntreprise IN (SELECT IDEntreprise FROM Entreprise WHERE Ent_Nom = "Flashset"))) as t;
 ...
 */
SELECT * FROM Produit WHERE IDProduit IN (
SELECT IDProduit FROM ProduitParCommande WHERE IDCommande LIKE (
SELECT IDCommande FROM Commande WHERE Commande.IDClient LIKE (
SELECT IDClient FROM Client WHERE Cli_Email = 'nlane7@digg.com') AND Com_DateLivraison='2014-11-09'));
...
WHERE Pro_Nom = "congue" ORDER BY PPL_PrixUnitaireHTAchat;
